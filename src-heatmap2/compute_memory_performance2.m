function [performance] = compute_memory_performance2(images, V_line, T_Iapp)
    ...
    for i = 1:params.max_spikes_thr
        ...
        mean_similarities(i) = mean(whole_similarity);
    end
    ...
    [performance.learned_similarities, performance.learned_similarities_pattern, performance.learned_similarities_background] = ...
        compute_images_similarity(images, ...
            performance.spike_images_best_thr, ...
            learned_patterns);
end

function [similarity, similarity_pattern, similarity_background] = compute_image_similarity(true_image, estimated_image)
    pattern_mask = true_image == 1;
    background_mask = true_image == 0;
%     n_pattern = sum(pattern_mask, 'all');
    n_pattern = sum(sum(pattern_mask));
%     n_background = sum(background_mask, 'all');
    n_background = sum(sum(background_mask));
    ...
    similarity_pattern = n_true_pattern / n_pattern;
    similarity_background = n_true_background / n_background;
    similarity = (similarity_background + similarity_pattern) / 2;
%     fprintf('True: %0.4f\n', n_true_pattern / n_pattern);
%     fprintf('Background: %0.4f\n', n_true_background / n_background);
end

function [similarity, similarity_pattern, similarity_background] = compute_images_similarity(images, spike_images, test_patterns)
    similarity = zeros(length(test_patterns), 1);
    similarity_pattern = zeros(length(test_patterns), 1);
    similarity_background = zeros(length(test_patterns), 1);
    for k = 1:length(test_patterns)
        estimated_image = spike_images(:,:,k);
        true_image = images{test_patterns(k)} < 127;
        [similarity(k), similarity_pattern(k), similarity_background(k)] = compute_image_similarity(true_image, estimated_image);
    end
end